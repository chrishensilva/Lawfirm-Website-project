...
export default function App() {
  const [view, setView] = useState('home'); // 'home' | 'about' | 'services' | 'team' | 'blog' | 'contact' | 'portal'
  ...
  const selectAttorney = (attorneyName) => {
    setSelectedAttorney(attorneyName);
  };

  const clearSelectedAttorney = () => {
    setSelectedAttorney(null);
  };
  ...
  const getMetaData = () => {
    switch (view) {
      ...
      case 'team':
        return {
          title: "Our People | Vohara Legal",
          desc: "Meet our qualified legal professionals with extensive experience in civil, commercial, property, and criminal practice in Sri Lanka."
        };
      case 'blog':
        return {
      ...
      case 'portal':
        return {
          title: "Secure Client Case Portal | Vohara Legal",
          desc: "Access your secure case portal. Download shared documents, message lead attorneys, track case milestones, and manage retainer payments."
        };
      default:
        return {
  ...
  return (
    <>
      ...
      {/* Main Content Area */}
      <main ref={mainRef} style={{ minHeight: '80vh' }}>
        {view === 'home' && (
          <>
        ...
        {view === 'team' && <Team setView={setView} selectAttorney={selectAttorney} />}
        {view === 'blog' && <Blog />}
        {view === 'contact' && (
          <Contact 
            selectedAttorney={selectedAttorney} 
            clearSelectedAttorney={clearSelectedAttorney}
            setView={setView}
          />
        )}
        {view === 'portal' && <Portal />}
